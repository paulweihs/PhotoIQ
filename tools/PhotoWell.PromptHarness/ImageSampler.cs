...
/// <summary>
/// Queries the PhotoWell app's SQLite library (read-only) to sample photos
...
public static class ImageSampler
{
    ...
    private static string DefaultPhotoWellDb =>
        Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            "PhotoWell", "photowell.db");

    public static List<SampleImage> SampleAdditive(int count, HashSet<string> exclude, string? photoIQDbPath = null)
    {
        photoIQDbPath ??= DefaultPhotoWellDb;

        if (!File.Exists(photoIQDbPath))
        {
            Console.WriteLine($"[ERROR] PhotoWell library not found: {photoIQDbPath}");
            return [];
        }
        ...
        cmd.CommandText = """
            ...
            LIMIT $limit
            """;
        cmd.Parameters.AddWithValue("$limit", (count + exclude.Count) * 5);
        ...
        while (reader.Read() && results.Count < count)
        {
            ...
            exclude.Add(fileName); // prevent duplicates within this batch
        }

        Console.WriteLine($"[Sampler] {results.Count}/{count} new photos sampled (excluding {exclude.Count - results.Count} existing).");
        return results;
    }
    ...
    public static List<SampleImage> Sample(int count, string? photoIQDbPath = null)
    {
        ...
        {
            Console.WriteLine($"[ERROR] PhotoWell library not found: {photoIQDbPath}");
            Console.WriteLine("        Run PhotoWell at least once to create the library.");
            return [];
        }
