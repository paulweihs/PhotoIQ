// PhotoWell.PromptHarness
//
// Standalone prompt engineering + regression test harness for the llama3.2-vision prompt.
// Independent of the PhotoWell app — reads PhotoWell's SQLite library read-only for sampling.
//
// WORKFLOW:
...
return command switch
{
    ...
    "--expand"             => await ExpandAsync(db, count, photoIQDb),
...
async Task<int> ExpandAsync(HarnessDb db, int n, string? srcDb)
{
    int existing = db.GetSampleCount();
    if (existing == 0)
    {
        Console.WriteLine("[ERROR] No existing sample. Run --init first.");
        return 1;
    }
    var exclude = db.GetExistingFileNames();
    Console.WriteLine($"[INFO] Expanding corpus by {n} images (currently {existing}, excluding {exclude.Count} already sampled).");
    var images = await Task.Run(() => ImageSampler.SampleAdditive(n, exclude, srcDb));
    if (images.Count == 0) { Console.WriteLine("[ERROR] No new images found."); return 1; }
    foreach (var img in images)
        db.InsertSampleImage(img);
    Console.WriteLine($"[OK] Added {images.Count} images. Total corpus: {existing + images.Count}.");
    Console.WriteLine($"     Next: run --export-manifest, generate gt.json for the new images, then --import-gt.");
    return 0;
}
...
string? FindAppSettings()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (dir == null) break;
        string candidate = Path.Combine(dir, "src", "PhotoWell.Common", "AppSettings.cs");
...
int ShowResults(HarnessDb db, GatesConfig gates, string? rid, int limit)
{
    ...
    foreach (var r in results.OrderBy(r => r.TotalScore))
    {
        ...
        if (gt != null)
        {
            ...
            Console.WriteLine($"  GT key_objects: {string.Join(", ", gt.KeyObjects)}");
        }
...
{
    Console.WriteLine("""
        PhotoWell.PromptHarness — Vision prompt engineering & regression testing
