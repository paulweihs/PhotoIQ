...
public static class GateEvaluator
{
    ...
    private static int ScoreKeywordMatch(string lower, GroundTruth gt, GateConfig gate)
    {
        ...
        // For setting keywords, treat inside/outside as synonyms of indoor/outdoor —
        // the model naturally writes either form; both are semantically correct.
        string normalizedLower = gate.SourceField == "setting"
            ? lower.Replace("inside", "indoor").Replace("outside", "outdoor")
            : lower;

        int matched = keywords.Count(k => normalizedLower.Contains(k));
    ...
    private static int ScorePersonCount(string lower, GroundTruth gt, GateConfig gate)
    {
        ...
        // Large groups (≥8): accept exact digit/word OR a "large group" / "group of" phrase —
        // models cannot reliably count 8+ people precisely; requiring the exact digit is unrealistic.
        if (gt.PersonCount >= 8)
        {
            string expected8 = gt.PersonCount.ToString();
            string written8  = NumberWord(gt.PersonCount);
            if (lower.Contains(expected8) || (!string.IsNullOrEmpty(written8) && lower.Contains(written8)))
                return gate.Points;
            if (lower.Contains("large group") || lower.Contains("group of"))
                return gate.Points;
            return 0;
        }

        // Small/medium groups (1-7): check for digit or written-out number
