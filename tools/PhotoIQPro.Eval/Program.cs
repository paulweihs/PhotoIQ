...
if (args.Length == 0)
{
    ...
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --corpus-status [--eval-db <path>]");
    Console.WriteLine("    Shows how many images are in the reference corpus, broken down by decade.");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --rebuild-corpus [--eval-db <path>] [--main-db <path>]");
    ...
    Console.WriteLine("    --main-db       Path to photoiq.db (default: %LOCALAPPDATA%\\PhotoIQPro\\photoiq.db)");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --build-corpus [count] [--eval-db <path>] [--main-db <path>]");
    ...
    Console.WriteLine("    --main-db       Path to photoiq.db (default: %LOCALAPPDATA%\\PhotoIQPro\\photoiq.db)");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --list-unprocessed [count] [--db <path>] [--eval-db <path>]");
    Console.WriteLine("    count           Number of images to list (default: 50).");
    Console.WriteLine("    --db            Path to photoiq.db.");
    Console.WriteLine("    --eval-db       Path to evaluations.db (default: ./evaluations.db).");
    Console.WriteLine("    Outputs one line per image: imagePath|||fileName|||filePath|||year");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --ingest-description --image <path> --file <path> --name <name> --desc-file <path> [--eval-db <path>] [--year <yyyy>]");
    Console.WriteLine("    Reads a Claude-generated description from --desc-file and persists it to the corpus.");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --optimize-prompt [count] --prompt <text> [--model llama3.2-vision] [--db <path>] [--eval-db <path>]");
    ...
    Console.WriteLine("    --eval-db       Path to evaluations.db (default: ./evaluations.db).");
    Console.WriteLine();
    Console.WriteLine("  PhotoIQPro.Eval --ensemble [count] --prompt <text> --models <model1,model2,...> [--eval-db <path>]");
...
if (args[0] == "--accept-prompt")
{
    ...
    if (acceptDb.AcceptPromptOptRun(acceptRunId))
    {
        ...
        Console.WriteLine("[TODO] Update AppSettings.VisionAnalysisPrompt in PhotoIQPro.Common");
