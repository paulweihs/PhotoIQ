...
public class ImportServiceTests
{
    ...
    [InlineData("/photos/video.mp4", false)]
    ...
    // ── Regression: AiModelUsed must use AppSettings.VisionModelName ─────────
    // Bug: UserPreferences.Current.VisionModelName (default "llama3.2-vision") was
    // being written to AiModelUsed even though inference runs on AppSettings.VisionModelName
    // ("minicpm-v"). This caused every photo to appear outdated immediately after analysis.
    // Fixed 2026-05-03; these tests prevent reintroduction.

    [Fact]
    public async Task RunVisionForPhotoAsync_StoresAppSettingsModelName_NotUserPreferences()
    {
        // Arrange — temp file so File.Exists passes, suitable dimensions so vision fires.
        var tmpFile = Path.GetTempFileName() + ".jpg";
        File.WriteAllBytes(tmpFile, new byte[100]);
        try
        {
            var fileId = Guid.NewGuid();
            MediaFile? captured = null;
            ...
            var photo = new MediaFile
            {
                ...
                DateImported   = DateTime.UtcNow,
                Tags           = new List<Tag>(),
                AnalysisStatus = AnalysisStatus.VisionPending,
                Width          = 800,
                Height         = 600,
                ThumbnailLarge = tmpFile,   // use the temp file as the "thumbnail" to skip preprocessing
            };

            _mockRepo.Setup(r => r.GetByIdAsync(fileId)).ReturnsAsync(photo);
            _mockRepo.Setup(r => r.UpdateAsync(It.IsAny<MediaFile>()))
                     .Callback<MediaFile>(mf => captured = mf)
                     .Returns(Task.CompletedTask);

            _mockVision.Setup(v => v.AnalyzeImageAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(new ImageUnderstanding("A dog sits on a green chair in a living room.", []));
            _mockVision.Setup(v => v.PromptVersion).Returns(AppSettings.CurrentPromptVersion);

            // Act
            await _service.RunVisionForPhotoAsync(fileId);

            // Assert — AiModelUsed must be AppSettings.VisionModelName ("minicpm-v"),
            // NOT UserPreferences.Current.VisionModelName (which defaults to "llama3.2-vision").
            Assert.NotNull(captured);
            Assert.Equal(AppSettings.VisionModelName, captured!.AiModelUsed);
            Assert.NotEqual("llama3.2-vision", captured.AiModelUsed);
        }
        finally
        {
    ...
    [Fact]
    public async Task ReanalyzeFileAsync_StoresAppSettingsModelName_NotUserPreferences()
    {
        // Arrange — temp file so drive/file checks pass.
        var tmpFile = Path.GetTempFileName() + ".jpg";
        File.WriteAllBytes(tmpFile, new byte[100]);
        try
        {
            ...
            _mockRepo.Setup(r => r.GetOrCreateTagAsync(
                    ...
                    It.IsAny<TagCategory>(), It.IsAny<bool>(), It.IsAny<float>()))
                .ReturnsAsync(new Tag { Id = Guid.NewGuid(), Name = "dog", NormalizedName = "dog", IsAIGenerated = true });
            _mockTagging.Setup(t => t.IsAvailable).Returns(false);
            _mockFaces.Setup(f => f.DetectFacesAsync(It.IsAny<MediaFile>(), It.IsAny<CancellationToken>()))
                      .ReturnsAsync(false);
            _mockThumbs.Setup(t => t.GenerateThumbnailsAsync(It.IsAny<MediaFile>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(new ThumbnailResult(false, null, null, null, null));

            _mockVision.Setup(v => v.IsAvailable).Returns(true);
            _mockVision.Setup(v => v.AnalyzeImageAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(new ImageUnderstanding("A fresh description of a photo.", []));
            _mockVision.Setup(v => v.PromptVersion).Returns(AppSettings.CurrentPromptVersion);

            // Act
            await _service.ReanalyzeFileAsync(fileId);

            // Assert — the last UpdateAsync call should have AiModelUsed = AppSettings.VisionModelName.
            Assert.NotNull(captured);
            Assert.Equal(AppSettings.VisionModelName, captured!.AiModelUsed);
            Assert.NotEqual("llama3.2-vision", captured.AiModelUsed);
        }
