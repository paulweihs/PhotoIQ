...
public class NeutraliseGenderTests
{
    // Nouns — singular
    [Theory]
    [InlineData("A woman walks by.",   "A person, who appears to be a woman, walks by.")]
    [InlineData("A man waves hello.",  "A person, who appears to be a man, waves hello.")]
    [InlineData("A girl is playing.",  "A child, who appears to be a girl, is playing.")]
    [InlineData("A boy holds a kite.", "A child, who appears to be a boy, holds a kite.")]
    ...
    [Theory]
    [InlineData("Two women are talking.",    "Two people, who appear to be women, are talking.")]
    [InlineData("Several men stand there.",  "Several people, who appear to be men, stand there.")]
    [InlineData("Three girls are laughing.", "Three children, who appear to be girls, are laughing.")]
    [InlineData("Four boys run around.",     "Four children, who appear to be boys, run around.")]
    ...
    public void MenNotDoubleReplaced()
    {
        ...
        Assert.Equal("The people, who appear to be men, walked in.", result);
    }
    ...
    public void WomenNotDoubleReplaced()
    {
        ...
        Assert.Equal("The people, who appear to be women, cheered.", result);
    }
...
// ── CollapsePersonPhrases (tested via NeutraliseGender) ──────────────────────

/// <summary>
/// Tests for the private CollapsePersonPhrases method, exercised through
/// NeutraliseGender which calls it as the first pipeline step.
/// </summary>
public class CollapsePersonPhrasesTests
{
    // ── Two-person compounds → "two people" ──────────────────────────────────

    [Fact]
    public void ManAndWoman_BecomeTwoPeople()
    {
        var result = TextNormalizer.NeutraliseGender("A man and a woman stood together.");
        Assert.DoesNotContain("a person and a person", result, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("two people", result, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void WomanAndMan_BecomeTwoPeople()
    {
        var result = TextNormalizer.NeutraliseGender("A woman and a man smiled.");
        Assert.DoesNotContain("a person and a person", result, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("two people", result, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void ManAndMan_BecomeTwoPeople()
    {
        var result = TextNormalizer.NeutraliseGender("A man and a man walked.");
        Assert.Contains("two people", result, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void WomanAndWoman_BecomeTwoPeople()
    {
        var result = TextNormalizer.NeutraliseGender("A woman and a woman sat.");
        Assert.Contains("two people", result, StringComparison.OrdinalIgnoreCase);
    }

    // ── Three-person compounds → "three people" ───────────────────────────────

    [Fact]
    public void ManCommaWomanCommaAndMan_BecomeThreePeople()
    {
        var result = TextNormalizer.NeutraliseGender("A man, a woman, and a man walked in.");
        Assert.Contains("three people", result, StringComparison.OrdinalIgnoreCase);
    }

    // ── Single person — no collapsing, just individual noun substitution ───────

    [Fact]
    public void SingleMan_BecomesPerson_NoCollapse()
    {
        var result = TextNormalizer.NeutraliseGender("A man waved.");
        Assert.Equal("A person, who appears to be a man, waved.", result);
    }

    // ── Already-plural form — no crash, reasonable output ────────────────────

    [Fact]
    public void AlreadyPluralMenAndWoman_NoException_OutputReasonable()
    {
        // "Two men and a woman" — "men" is not "a man" so the two-person collapse
        // regex does not fire. "men" → "people", "woman" → "person" individually.
        var ex = Record.Exception(() => TextNormalizer.NeutraliseGender("Two men and a woman walked."));
        Assert.Null(ex);

        var result = TextNormalizer.NeutraliseGender("Two men and a woman walked.");
        // "men" appears in the hint "who appear to be men" — assert the neutral noun "people" is present
        Assert.Contains("people", result, StringComparison.OrdinalIgnoreCase);
    }

    // ── Post-substitution fallback cleanup ────────────────────────────────────

    [Fact]
    public void PersonAndPerson_FallbackRegex_BecomeTwoPeople()
    {
        // The post-substitution cleanup at the end of NeutraliseGender catches any
        // "a person and a person" that slipped through (e.g. if the input already
        // used the word "person" before NeutraliseGender ran).
        var result = TextNormalizer.NeutraliseGender("A person and a person sat down.");
        Assert.Contains("two people", result, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sat down", result, StringComparison.OrdinalIgnoreCase);
    }

    // ── Sentence-start capitalisation after collapse ───────────────────────────

    [Fact]
    public void TheyCapitalisedAtSentenceStart_AfterPronounSubstitution()
    {
        // "She walks. She smiles." → pronoun step produces "They walks. They smiles."
        // The re-capitalisation step ensures "They" after ". " is capital.
        var result = TextNormalizer.NeutraliseGender("She walks to the park. She smiles warmly.");
        // The second "They" must start with a capital after the period+space boundary.
        int periodIndex = result.IndexOf(". ", StringComparison.Ordinal);
        Assert.True(periodIndex >= 0, "Expected a period followed by a space");
        var afterPeriod = result[(periodIndex + 2)..];
        Assert.True(char.IsUpper(afterPeriod[0]), $"Expected capital after '. ' but got: {afterPeriod}");
    }

    [Fact]
    public void TheyCapitalised_AfterPeriodSpace_MultiSentence()
    {
        // Multi-sentence: the re-capitalisation regex (?<=\.\s+)they → They must fire.
        var result = TextNormalizer.NeutraliseGender("they walked together. They were happy.");
        // After period+space, "They" must be capital (the input already has it capital,
        // but the verb-fix "they were" lowercases the first occurrence; confirm the
        // second sentence starts with a capital regardless).
