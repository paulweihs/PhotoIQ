...
public partial class FaceReviewDialog : Window
{
    ...
    public FaceReviewDialog(IReadOnlyList<(Face, MediaFile)> queue, string personName, PhotoViewerViewModel vm)
    {
        InitializeComponent();
        _queue = queue;
    ...
    /// <summary>User confirmed this face — mark the photo as reviewed and advance.</summary>
    private async void YesBtn_Click(object sender, RoutedEventArgs e)
    {
        if (_index >= _queue.Count) return;
        var (_, photo) = _queue[_index];
