...
/// - Model loading (clip-vit-base-patch32-vision.onnx from %LOCALAPPDATA%\PhotoWell\models\)
...
public class ClipEngine : IDisposable
{
    ...
    /// </remarks>
    /// <param name="path">Optional custom path to the models directory. If null, defaults to %LOCALAPPDATA%\PhotoWell\models.</param>
    public ClipEngine(string? path = null) => _modelsPath = path ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PhotoWell", "models");
