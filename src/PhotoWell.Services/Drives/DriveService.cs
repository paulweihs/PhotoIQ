// File: PhotoWell.Services/Drives/DriveService.cs
using System;
...
namespace PhotoWell.Services.Drives;
...
public class DriveService : IDriveService
{
    ...
    public IReadOnlyList<string> DefaultExclusions { get; } = new List<string>
    {
        ...
        // App-specific
        "PhotoWell"   // Don't scan our own app data
    }.AsReadOnly();
        ...
        IReadOnlyCollection<string>? excludedPaths = null)
    {
        ...
        var pathExclusionSet = excludedPaths is null
            ? new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            : new HashSet<string>(excludedPaths, StringComparer.OrdinalIgnoreCase);

        await Task.Run(() =>
        {
            ScanDirectory(path, recursive, exclusionSet, pathExclusionSet, files, ref foldersScanned, progress, ct);
        }, ct);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Check if this full path (or any ancestor) matches a path exclusion
            if (pathExclusions.Count > 0)
            {
                var normalized = directory.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
                if (pathExclusions.Any(ex =>
                    normalized.Equals(ex.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar), StringComparison.OrdinalIgnoreCase) ||
                    normalized.StartsWith(ex.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase)))
                    return;
            }
            ...
            if (recursive)
            {
                ...
                {
                    ScanDirectory(subDir, true, exclusions, pathExclusions, results, ref foldersScanned, progress, ct);
                }
            }
        }
