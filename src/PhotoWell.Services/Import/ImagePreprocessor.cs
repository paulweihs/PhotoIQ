...
{
    // Read live from user prefs so changes take effect without restart.
    // Clamped to a safe range so a corrupt settings value can't cause issues.
    private static int MaxVisionDimension => Math.Clamp(UserPreferences.Current.VisionImageSize, 256, 1024);
    ...
    public async Task<PreparedImage?> PrepareAsync(string filePath, CancellationToken ct = default)
    {
        ...
        var tempPath = Path.Combine(Path.GetTempPath(), $"photowell_{Guid.NewGuid():N}.jpg");
        ...
        // WIC decodes at full resolution; resize to cap before saving.
        try
        {
            var bytes = await ToJpegViaWicAsync(filePath, ct);
            if (bytes is null) return null;
            int maxDim = MaxVisionDimension;
            using var ms     = new MemoryStream(bytes);
            using var wicImg = await Image.LoadAsync(ms, ct);
            if (Math.Max(wicImg.Width, wicImg.Height) > maxDim)
                wicImg.Mutate(x => x.Resize(new ResizeOptions
                {
                    Size = new Size(maxDim, maxDim),
                    Mode = ResizeMode.Max
                }));
            await wicImg.SaveAsJpegAsync(tempPath, new JpegEncoder { Quality = 85 }, ct);
            return new PreparedImage(tempPath, isTemp: true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
