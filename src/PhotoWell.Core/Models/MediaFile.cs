...
public class MediaFile : INotifyPropertyChanged
{
    ...
    /// <summary>DCT-based perceptual hash (pHash) for near-duplicate detection. NULL until computed at import.</summary>
    ...
    /// <summary>PhotoWell version string at the time this file was imported (e.g. "0.2.3"). Null for files imported before version tracking.</summary>
    public string? ImportedWithVersion { get; set; }
    /// <summary>PhotoWell version string at the time AI analysis completed (e.g. "0.2.3"). Null for files analyzed before version tracking.</summary>
    public string? AnalyzedWithVersion { get; set; }
    /// <summary>User-applied rotation in degrees (0, 90, 180, 270). Applied on top of EXIF orientation when generating thumbnails. Default 0.</summary>
    private int _userRotation;
    public int UserRotation { get => _userRotation; set { if (_userRotation == value) return; _userRotation = value; Notify(); } }
    ...
    /// <summary>True when the photo has not yet been analyzed at all (Pending) or is queued for AI description (VisionPending).
    /// Used to show a pending indicator badge on the thumbnail.</summary>
    [NotMapped]
